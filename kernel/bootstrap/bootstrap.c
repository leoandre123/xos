...
extern ulong get_kernel_main_byte0(void);
extern ulong get_kernel_main_byte1(void);
extern ulong get_kernel_main_byte2(void);
extern ulong get_kernel_main_byte3(void);
...
BOOT_CODE void bootstrap_main(BootInfo *boot_info) {
  ...
  for (ulong i = 0; i < 512; ++i) {
    g_low_pd[i] = (i * PAGE_SIZE_2M) | PAGE_PRESENT | PAGE_WRITABLE | PAGE_HUGE;
  }
  ...
  ulong page_count = (phys_end - phys_start) / PAGE_SIZE;
  ...
  for (ulong i = 0; i < page_count; ++i) {
    g_high_pt[i] = (phys_start + i * PAGE_SIZE) | PAGE_PRESENT | PAGE_WRITABLE;
  }
  ...
  boot_serial_write_hex64(get_kernel_main_byte0());
  serial_write_boot(STR_NEWLINE);
  boot_serial_write_hex64(get_kernel_main_byte1());
  serial_write_boot(STR_NEWLINE);
  boot_serial_write_hex64(get_kernel_main_byte2());
  serial_write_boot(STR_NEWLINE);
  boot_serial_write_hex64(get_kernel_main_byte3());
  serial_write_boot(STR_NEWLINE);
