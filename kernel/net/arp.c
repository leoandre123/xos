...
static void arp_reply_ipv4(ipv4_addr from_addr, mac_addr from_mac, nic *nic) {
  arp_packet_ipv4 arp_packet = {
      ...
      .sender_ip_address = nic->addr,
      .target_mac_address = from_mac,
      .target_ip_address = from_addr,
  };
  e1000_get_mac(&arp_packet.sender_mac_address);

  ethernet_send(from_mac, ETHERTYPE_ARP, &arp_packet, sizeof(arp_packet_ipv4), nic->driver);
}
...
void arp_receive(ubyte *data, ushort len, nic *nic) {
  ...
  if (op == ARP_REPLY) {
    ...
    ethernet_driver_flush_pending(sender_ip);
    ...
    if (packet->target_ip_address.value == nic->addr.value)
      arp_reply_ipv4(sender_ip, sender_mac, nic);
  }
}
...
void arp_send_ipv4(ipv4_addr addr, nic *nic) {
  ...
  e1000_get_mac(&arp_packet.sender_mac_address);
  arp_packet.sender_ip_address = nic->addr;
  ...
  ethernet_send(broadcast, ETHERTYPE_ARP, &arp_packet, sizeof(arp_packet_ipv4), nic->driver);
}
