...
// Load kernel.bin via TFTP when booted over PXE (no EFI filesystem on device handle).
static EFI_STATUS pxe_load_file(EFI_HANDLE image_handle,
  ...
  UINT64 file_size = 0;
  status = uefi_call_wrapper(pxe->Mtftp, 10, pxe,
                             EFI_PXE_BASE_CODE_TFTP_GET_FILE_SIZE,
                             NULL, FALSE, &file_size, NULL,
  ...
  status = uefi_call_wrapper(BS->AllocatePages, 4, AllocateAddress,
                             EfiLoaderData, EFI_SIZE_TO_PAGES(file_size), &addr);
  if (EFI_ERROR(status)) return status;
...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
  ...
  // Detect boot device: walk the device path to determine type and PCI location,
  // then read GPT to find the data partition start LBA.
  EFI_LOADED_IMAGE *loaded_image = NULL;
  EFI_GUID li_guid = EFI_LOADED_IMAGE_PROTOCOL_GUID;
  ...
  if (loaded_image) {
    ...
    EFI_STATUS gpt_status = uefi_call_wrapper(BS->LocateHandleBuffer, 5,
        ByProtocol, &bio_guid, NULL, &num_handles, &handles);
    ...
    if (!EFI_ERROR(gpt_status)) {
      for (UINTN i = 0; i < num_handles; i++) {
        ...
        if (EFI_ERROR(uefi_call_wrapper(bio->ReadBlocks, 5,
                bio, bio->Media->MediaId, 1, sizeof(gpt_hdr), gpt_hdr)))
          continue;
        ...
        bool is_gpt = true;
        for (int s = 0; s < 8; s++) {
          if (gpt_hdr[s] != sig[s]) { is_gpt = false; break; }
        }
        if (!is_gpt)
          continue;

        // PartitionEntryLBA at offset 72 (8 bytes), entry size at offset 84 (4 bytes),
        // num entries at offset 80 (4 bytes).
        ...
        if (EFI_ERROR(uefi_call_wrapper(BS->AllocatePool, 3,
                EfiLoaderData, entries_bytes, (void **)&entries)))
          continue;

        UINTN lbas_needed = (entries_bytes + 511) / 512;
        UINT8 *entry_buf = NULL;
        uefi_call_wrapper(BS->AllocatePool, 3,
            EfiLoaderData, lbas_needed * 512, (void **)&entry_buf);

        bool found = false;
        if (entry_buf && !EFI_ERROR(uefi_call_wrapper(bio->ReadBlocks, 5,
                bio, bio->Media->MediaId, entry_lba,
                lbas_needed * 512, entry_buf))) {
          // ESP partition type GUID (little-endian):
          // C12A7328-F81F-11D2-BA4B-00A0C93EC93B
          const UINT8 esp_guid[16] = {
          ...
          for (UINT32 p = 0; p < num_entries && !found; p++) {
            ...
            bool is_esp = true;
            for (int g = 0; g < 16; g++) {
              if (ent[g] != esp_guid[g]) { is_esp = false; break; }
            }
            if (is_esp) continue;
        ...
        if (entry_buf) uefi_call_wrapper(BS->FreePool, 1, entry_buf);
